// Frontend Type Contracts — spec 016-tauri-app-shell
// TypeScript types matching Rust serialization for Tauri invoke() and events.

// --- Core Event (single channel: "core-event") ---

// Adjacently tagged: { type: "download_progress", data: { ... } }
export type CoreEvent =
    ...
    | {
          type: "scan_progress";
          data: { progress: number; current_id: string };
      }
    ...
    | {
          ...
          data: { package_id: string; status: string };
      }
    | {
...
// SoftwareEntry, SearchResult, ScanResult, BackupInfo, RestoreResult, AppConfig
// are defined in astro-up-core and serialized as JSON. Their exact shapes are
// determined by the core crate's serde output — generate with `cargo test`
// snapshot tests and sync to frontend types.

// --- Error log (frontend-only state) ---
