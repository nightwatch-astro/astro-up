...
- **FR-006**: The compiler MUST produce a SQLite database (`catalog.db`) with a hybrid schema: a `packages` table (metadata columns + JSON columns for nested configs: detection, install, checkver, dependencies, hardware, backup) with indexes on `category`, `type`, `slug` and FTS5 for fuzzy search on `name`, `description`, `tags`, `publisher`; a separate `versions` table (`package_id`, `version`, `url`, `sha256`, `discovered_at`, `release_notes_url`, PRIMARY KEY on `package_id + version`) for efficient version queries and delta sync; and a `meta` table for schema version and compilation timestamp
...
- Q: SQLite catalog schema — normalized, hybrid, or denormalized? → A: Hybrid Option B with separate versions table. `packages` table has metadata columns + JSON for nested configs (detection, install, checkver). Separate `versions` table for efficient version queries, delta sync, and rollback browsing. FTS5 for fuzzy search. `meta` table for schema version.
