...
- **FR-025**: `Downloader` trait via `#[trait_variant::make(DownloaderDyn: Send)]`: `async fn download(&self, url: &str, dest: &Path, opts: &DownloadOptions) -> Result<(), CoreError>`. Engine uses `Box<dyn DownloaderDyn>`.
